...
Each machine with an age key pair is listed in `.sops.yaml` and can decrypt `modules/aspects/secrets/secrets.yaml`.
Any such machine can rebuild from a fresh clone without needing keys from other machines.
